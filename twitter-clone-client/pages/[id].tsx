...
import { followUserMutation, un } from "@/graphql/mutation/user";
...
const UserProfilePage: NextPage<ServerProps> = (props) => {
  ...
  const amIfollowing=useMemo(()=>{
    ...
    return ( props.userInfo.followers?.findIndex((e)=>e?.id===currentUser?.id)??-1  )>=0

  },[props.userInfo, currentUser?.id]);
          ...
          <div className="p-4 border-b border-slate-800">
            ...
            </div>
            {
              ...
              (
                <>
                  {amIfollowing? (
                    <button className=" bg-[#1d9bf0] text-white px-3 py-1 rounded-full text-sm ">
                      Unfollow</button>
                  ):(
                    <button onClick={handleFollowUser} className=" bg-[#1d9bf0] text-white px-3 py-1 rounded-full text-sm ">
                      Follow</button>
